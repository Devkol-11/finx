...
export class WalletService {
  ...
  public async getBalance(userId: string, input: BalanceQueryInput) {
    const { wallet, recentActivity } =
      await this.walletRepository.getBalanceWithRecentActivity(userId, input);
  ...
  public async initiateFiatDeposit(
    userId: string,
    email: string,
    input: DepositInput,
    ...
    try {
      ...
      const updatedIntent = await this.walletRepository.markDepositInitialized(
  ...
  public async withdrawFiat(
    ...
    try {
      ...
      const providerIntent =
        await this.walletRepository.attachWithdrawalProviderResult(
      ...
      if (providerResult.status === "success") {
        const result =
          await this.walletRepository.settleSuccessfulFiatWithdrawal(
            reservation.paymentIntent.reference,
            providerResult.raw
          );
      ...
      await this.walletRepository.releaseFailedFiatWithdrawal(
        ...
        error instanceof Error
          ? error.message
          : "Payment provider failed to process withdrawal.",
        error instanceof Error ? { message: error.message } : undefined
      );
      throw this.wrapProviderError(error);
    }
  ...
  public async verifyFiatDeposit(userId: string, reference: string) {
    const paymentIntent = await this.walletRepository.findPaymentIntentForUser(
      userId,
      reference
    );
    ...
    if (paymentIntent.type !== PaymentType.FIAT_DEPOSIT) {
      throw AppError.badRequest(
        "Only fiat deposits can be verified through this endpoint."
      );
    }
    ...
    if (verification.status === "success") {
      const result = await this.walletRepository.postSuccessfulFiatDeposit(
        reference,
        verification
      );
    ...
    if (
      ...
      await this.walletRepository.markPaymentFailed(
        ...
        verification.gatewayResponse ??
          `Paystack marked transaction as ${verification.status}.`,
        verification.raw
      );
    }

    const refreshedIntent =
      await this.walletRepository.findPaymentIntentForUser(userId, reference);
  ...
  public async handlePaystackWebhook(input: {
    reference: string;
    eventName: string;
    payload: unknown;
  }) {
    const paymentIntent = await this.walletRepository.recordPaymentWebhook(
      input
    );
    ...
    if (
      paymentIntent.type === PaymentType.FIAT_DEPOSIT &&
      input.eventName === "charge.success"
    ) {
      const verification = await this.paymentProvider.verifyTransaction(
      ...
      if (verification.status === "success") {
        await this.walletRepository.postSuccessfulFiatDeposit(
          paymentIntent.reference,
          verification
        );
      }
    ...
    if (paymentIntent.type === PaymentType.FIAT_WITHDRAWAL) {
      ...
      if (
        input.eventName === "transfer.failed" ||
        input.eventName === "transfer.reversed"
      ) {
        await this.walletRepository.releaseFailedFiatWithdrawal(
          paymentIntent.reference,
  ...
  public async getTransactions(userId: string, input: TransactionsQueryInput) {
    const result = await this.walletRepository.getTransactionHistory(
      userId,
      input
    );
