...
import type { PortfolioQueryInput, SubscribeInput, WithdrawParamsInput } from './invest.schema';
...
export class InvestController {
  ...
  public subscribe = async (request: FastifyRequest<{ Body: SubscribeInput }>, reply: FastifyReply): Promise<void> => {
    const result = await this.investService.subscribe(request.user.userId, request.body);
    void reply.status(201).send(result);
  };

  public getPortfolio = async (request: FastifyRequest<{ Querystring: PortfolioQueryInput }>, reply: FastifyReply): Promise<void> => {
    const result = await this.investService.getPortfolio(request.user.userId, request.query);
    void reply.status(200).send(result);
  };

  public withdraw = async (request: FastifyRequest<{ Params: WithdrawParamsInput }>, reply: FastifyReply): Promise<void> => {
