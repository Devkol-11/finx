...
const isFastifyValidationError = (
  error: Partial<FastifyError> & { validation?: unknown[] },
): boolean => Array.isArray(error.validation);
    ...
    switch (error.code) {
      case "P2002":
        return buildResponse(
          409,
          "A unique resource already exists with the supplied value.",
      ...
      case "P2003":
        return buildResponse(
          409,
          "The requested operation violates a relational integrity constraint.",
  ...
  fastify.setErrorHandler(
    (error: unknown, request: FastifyRequest, reply: FastifyReply): void => {
      ...
      if (fastifyError.statusCode === 429) {
        const payload = buildResponse(
          429,
          fastifyError.message || "Too many requests.",
          "RATE_LIMIT_EXCEEDED",
        );
