...
export const walletApi = {
  ...
  withdraw: async (input: {
    ...
    const { data } = await apiClient.post<ApiEnvelope<{ reference: string; amount: string; walletBalance: string }>>(
      "/wallet/withdraw/fiat",
      { currency: "NGN", ...input },
    );
    return data;
  },
  transfer: async (input: { finxTag: string; amount: string; narration?: string }) => {
    const { data } = await apiClient.post<
      ApiEnvelope<{ reference: string; amount: string; receiver: { finxTag: string; email: string }; senderBalance: string }>
