...
export default function WithdrawPage() {
  ...
  const form = useForm<z.infer<typeof schema>>({
    ...
    defaultValues: { amount: '', bankCode: '', accountNumber: '', accountName: '', narration: '' }
  });
      ...
      <Card className="p-6">
        {mutation.isSuccess ? (
          ...
          <form
            ...
            onSubmit={form.handleSubmit((values) => mutation.mutate({ ...values, narration: values.narration || undefined }))}
          >
            {(['amount', 'bankCode', 'accountNumber', 'accountName'] as const).map((name) => (
              <FormField
                key={name}
                label={{ amount: 'Amount', bankCode: 'Bank code', accountNumber: 'Account number', accountName: 'Account name' }[name]}
