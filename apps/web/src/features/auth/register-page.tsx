...
const schema = z.object({
  ...
  password: z.string().min(8, "Use at least 8 characters").regex(/[A-Z]/, "Add an uppercase letter").regex(/[0-9]/, "Add a number").regex(/[^A-Za-z0-9]/, "Add a special character"),
});
...
export default function RegisterPage() {
  ...
  return (
    <Card className="p-6">
      ...
      <form className="mt-6 grid gap-4 sm:grid-cols-2" onSubmit={form.handleSubmit((values) => mutation.mutate({ ...values, phoneNumber: values.phoneNumber || undefined }))}>
      ...
      <p className="mt-5 text-sm text-slate-500">Already have an account? <Link className="font-semibold text-blue-700" to="/auth/login">Sign in</Link></p>
    </Card>
  );
}
